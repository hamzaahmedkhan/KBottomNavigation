...
internal typealias IBottomNavigationListener = (model: KBottomNavigation.Model) -> Unit

@Suppress("MemberVisibilityCanBePrivate")
class KBottomNavigation : FrameLayout {

    var models = ArrayList<Model>()
    var cells = ArrayList<KBottomNavigationCell>()
    ...
    private fun setAttributeFromXml(context: Context, attrs: AttributeSet) {
        val a = context.theme.obtainStyledAttributes(attrs, R.styleable.KBottomNavigation, 0, 0)
        try {
            a?.apply {
                defaultIconColor = getColor(R.styleable.KBottomNavigation_mbn_defaultIconColor, defaultIconColor)
                selectedIconColor = getColor(R.styleable.KBottomNavigation_mbn_selectedIconColor, selectedIconColor)
                backgroundBottomColor = getColor(R.styleable.KBottomNavigation_mbn_backgroundBottomColor, backgroundBottomColor)
                countTextColor = getColor(R.styleable.KBottomNavigation_mbn_countTextColor, countTextColor)
                countBackgroundColor = getColor(R.styleable.KBottomNavigation_mbn_countBackgroundColor, countBackgroundColor)
                val typeface = getString(R.styleable.KBottomNavigation_mbn_countTypeface)
                rippleColor = getColor(R.styleable.KBottomNavigation_mbn_rippleColor, rippleColor)
                shadowColor = getColor(R.styleable.KBottomNavigation_mbn_shadowColor, shadowColor)
    ...
    private fun initializeViews() {
        ...
        bezierView.apply {
            ...
            shadowColor = this@KBottomNavigation.shadowColor
        }
    ...
    fun add(model: Model) {
        val cell = KBottomNavigationCell(context)
        cell.apply {
            val params = LinearLayout.LayoutParams(0, heightCell, 1f)
            layoutParams = params
            icon = model.icon
            count = model.count
            circleColor = this@KBottomNavigation.backgroundBottomColor
            countTextColor = this@KBottomNavigation.countTextColor
            countBackgroundColor = this@KBottomNavigation.countBackgroundColor
            countTypeface = this@KBottomNavigation.countTypeface
            rippleColor = this@KBottomNavigation.rippleColor
            defaultIconColor = this@KBottomNavigation.defaultIconColor
            selectedIconColor = this@KBottomNavigation.selectedIconColor
            onClickListener = {
    ...
    private fun anim(cell: KBottomNavigationCell, id: Int, enableAnimation: Boolean = true) {
        isAnimating = true
    ...
    fun getCellById(id: Int): KBottomNavigationCell? {
    ...
    class Model(var id: Int, var icon: Int) {
        ...
        var count: String = KBottomNavigationCell.EMPTY_VALUE
